...
add_shortcode(
	...
	<form class="sims_form sims_taxform" action="<?php echo esc_url( $url ); ?>" method="get">
				...
				if ( isset( $atts['size'] ) ) {
					...
					$select  = str_replace( 'value="' . get_query_var( 'cat_sims' ) . '"', 'selected value=', $select ); // 値が選択中の場合!
		...
		<noscript>
			...
			$getqueryname = get_query_var( 'cat_sims' );
			if ( ! empty( $getqueryname ) ) {
				$style = 'style=display:block;';
			}
			$terms = get_categories( 'orderby=count&order=DESC' );
			?>
				<div class="sims_toglewrap<?php echo esc_attr( $class ); ?>">
					...
					foreach ( $terms as $term ) {
						...
						if ( $checked !== false ) {
							...
							echo '<label>' . esc_html( $term->name ) . '<input checked onChange="' . esc_attr( $url ) . '" type="checkbox" value="' . esc_url_raw( '?' . wp_parse_url( add_query_arg( array( 'cat_sims' => $checkedvalue ) ), PHP_URL_QUERY ) ) . '"></label>';
						} else {
							echo '<label>' . esc_html( $term->name ) . '<input onChange="' . esc_attr( $url ) . '" type="checkbox" value="' . esc_url_raw( '?' . wp_parse_url( add_query_arg( array( 'cat_sims' => $oldvalue . $term->term_id ) ), PHP_URL_QUERY ) ) . '"></label>';
						}
					}
					?>
					</div>
